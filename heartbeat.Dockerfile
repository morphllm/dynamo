...
ARG RUNTIME_IMAGE_TAG=v0.5.17-cu130-runtime

# wheel builder image selection

ARG WHEEL_BUILDER_IMAGE=quay.io/pypa/manylinux_2_28_aarch64
...
# --- SGLang Stages
    ...
    # SGLang builds directly from its upstream runtime image.
...
RUN --mount=type=secret,id=pip-index-url,env=PIP_INDEX_URL \
    --mount=type=secret,id=uv-default-index,env=UV_DEFAULT_INDEX \
    --mount=type=secret,id=pypi-netrc,target=/run/secrets/pypi-netrc,mode=0444 \
    --mount=type=bind,source=./container/deps/requirements.sglang.txt,target=/tmp/requirements.sglang.txt \
    --mount=type=cache,target=/root/.cache/pip,sharing=locked \
    export NETRC=/run/secrets/pypi-netrc && \
    export PIP_CACHE_DIR=/root/.cache/pip && \
...
# unfiltered).
ARG BASELINE_SBOM_FILE="cuda@8b2705ea"
ARG TARGETARCH
...
RUN PKG_ARG="--site-packages $(python3 -c 'import sysconfig; print(sysconfig.get_paths()["purelib"])')" && \
...
ARG TARGETARCH
RUN if [ "$ENABLE_SOURCE_ARCHIVAL" = "true" ]; then \
RUST_PKG_ARG="--rust-site-packages $(python3 -c 'import sysconfig; print(sysconfig.get_paths()["purelib"])')" && \
...
# Morph production overlay: keep axe1's exact qualified SGLang engine image and
# replace only Dynamo's compiled runtime plus the SGLang publisher source.
ARG HEARTBEAT_BASE_IMAGE
...
COPY --from=runtime_wheel_builder /opt/dynamo/components/src/dynamo/sglang/publisher.py /tmp/dynamo-heartbeat/publisher.py
RUN python3 -m pip install --no-deps --force-reinstall /tmp/dynamo-heartbeat/ai_dynamo_runtime-*.whl \
    && target=$(python3 -c 'import pathlib, dynamo.sglang.publisher as p; print(pathlib.Path(p.__file__))' 2>/dev/null | tail -n1) \
    && install -m 0644 /tmp/dynamo-heartbeat/publisher.py "$target" \
    && rm -rf /tmp/dynamo-heartbeat \
    && python3 -c 'from importlib.metadata import version; assert version("ai-dynamo") == "1.5.0.dev20260823"; assert version("ai-dynamo-runtime") == "1.5.0"; import dynamo.sglang.publisher as p; assert "every local data parallel rank" in open(p.__file__).read(); print("Dynamo idle load heartbeat overlay OK")'
