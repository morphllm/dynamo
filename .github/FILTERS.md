...
| `sidecar` | Multi-architecture unified sidecar container build; source and proto files also match `rust` |
...
> **Note:** `docs` and `ignore` don't directly trigger CI jobs. They exist to satisfy coverage requirements - every file must match at least one filter. Sidecar source and proto files also match `rust`, which runs the workspace Rust checks.
